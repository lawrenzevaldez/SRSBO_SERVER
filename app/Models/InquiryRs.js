...
class InquiryRs extends Model {
    ...
    async updateRmsHeader(deliveryName, plateNumber, rs_id, pictureInvoice, pictureRSSlip, pictureDriversID, pictureActualRS, user_id, rs_action, pictureInvoice_url, pictureRSSlip_url, pictureDriversID_url, pictureActualRS_url) {
        const trx = await Db.beginTransaction()
        try
        {
            if(rs_action == 2) {
                    ...
                    try
                    {
                        ...
                        if(row) {
                            ...
                            if(row) {
                                ...
                                await pictureInvoice.move(Helpers.publicPath(`images/${rs_id}`), {
                                ...
                                await pictureRSSlip.move(Helpers.publicPath(`images/${rs_id}`), {
                                    name: fpictureRSSlip,
                                    overwrite: true
                                })
                                ...
                                await pictureDriversID.move(Helpers.publicPath(`images/${rs_id}`), {
                                ...
                                await pictureActualRS.move(Helpers.publicPath(`images/${rs_id}`), {
    ...
    async updateRmsHeaderBulk(rs_id, user_id, rs_action) {
        ...
        try {
            if(rs_action == 2) {
                ...
                let row = await trx.select('trs_id')
                                .from('0_pickup_item')
                                .where('trs_id', rs_id)
                if(row.length > 0) {
                    return true
                } else {
                    let sel_temp = await trx.select('*')
                    ...
                    if(sel_temp.length > 0) {
                        ...
                        let transfer_row = await trx.insert(data)
                                                    .into('0_pickup_item')
                        ...
                        if(transfer_row) {
                            ...
                            await trx.table('0_pickup_item_temp')
                                .where('rs_id', rs_id)
                                .andWhere('rs_action', rs_action)
                                .update({status: 1})
                        }
    ...
    async getTImage(rs_id) {
        let row = await Db.select('tpictureActualRS', 'tpictureDriversID', 'tpictureInvoice', 'tpictureRSSlip', 'tpictureActualRS_url', 'tpictureDriversID_url', 'tpictureInvoice_url', 'tpictureRSSlip_url', 'tname', 'tplate_no')
                        .from('0_pickup_item')
                        .where('trs_id', rs_id)
    ...
    async createTempData(rs_id, rs_action) {
        try {
            ...
            let row = await Db.insert(data)
                    .into('0_pickup_item_temp')
    ...
    async uploadTempFileRsSlip(url, rs_id, rs_action, file, file_desc, user_id) {
        ...
        try
        {
            ...
            let row = await trx.table('0_pickup_item_temp')
                            .where('rs_id', rs_id)
                            .update(datas)
    ...
    async uploadTempFileActualRS(url, rs_id, rs_action, file, file_desc, user_id) {
        const trx = await Db.beginTransaction()
        try
        {
            // let rowChecker = await Db.select('*')
    ...
    async uploadTempFileDriversID(url, rs_id, rs_action, file, file_desc, user_id) {
        const trx = await Db.beginTransaction()
        try
        {
            // let rowChecker = await Db.select('*')
    ...
    async copyTempFileDriversID(url, rs_id, rs_action, rsid0, description, user_id) {
        const trx = await Db.beginTransaction()
        try {
            ...
            if(row == 1) {
                ...
                fs.copyFile(Helpers.publicPath(`images/${rsid0}/${fFile}`), Helpers.publicPath(`images/${rs_id}/${fFile}`), (error) => {
                    if(error) {
                        console.log(error)
                    }
                })
    ...
    async copyTempFileInvoice(url, rs_id, rs_action, rsid0, description, user_id) {
        const trx = await Db.beginTransaction()
        try {
            let fFile = `${user_id}~${description}.jpg`
